...
import { Network, DataSet, Options, Node, Edge } from 'vis-network';
...
export async function renderConceptMap(problemId: string, containerId: string): Promise<void> {
    ...
    try {
        ...
        const firestoreNodes: { id: string; text: string; color?: string; shape?: string }[] = nodesSnap.exists() ? nodesSnap.data().data : [];
        const firestoreEdges: { from: string; to: string; label?: string }[] = edgesSnap.exists() ? edgesSnap.data().data : [];
        ...
        const nodes = new DataSet<Node>(firestoreNodes.map(node => ({
            id: node.id,
            label: node.text,
        ...
        const edges = new DataSet<Edge>(firestoreEdges.map(edge => ({
            from: edge.from,
            to: edge.to,
            label: edge.label || '',
            arrows: 'to',
